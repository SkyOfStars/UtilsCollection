...
public class ExecutorServiceManager {
    ...
    private ExecutorService executorFixedService;
    private ExecutorService executorSingleService;
    private ExecutorService executorCachedService;
     ...
     * @param type 线程池类型
     */
    public ExecutorServiceManager getExecutorService(Type type) {
        switch (type) {
            case FixedThread:
                // 构造一个固定线程数目的线程池
                // ThreadPoolExecutor(corePoolSize, corePoolSize, 0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>());
                if (executorFixedService == null) {
                    executorFixedService = Executors.newFixedThreadPool(CORE_POOL_SIZE);
                }
                exec = executorFixedService;
                break;
            case SingleThread:
                ...
                exec = executorSingleService;
                break;
            case CachedThread:
                // 构造一个缓冲功能的线程池
                // ThreadPoolExecutor(0, Integer.MAX_VALUE, 60L, TimeUnit.SECONDS, new SynchronousQueue<Runnable>());
                if (executorCachedService == null) {
                    executorCachedService = Executors.newCachedThreadPool();
                }
                exec = executorCachedService;
                break;
    ...
    public void executeScheduledTask(Runnable command) {
        if (scheduledExecutorService == null) {
            scheduledExecutorService = Executors.newScheduledThreadPool(CORE_POOL_SIZE);
        }
    ...
    public ScheduledFuture<?> schedule(Runnable command, long delay, TimeUnit unit) {
        if (scheduledExecutorService == null) {
            getExecutorService(Type.ScheduledThread);
        }
